...
const AdminFacility = () => {
    ...
    const getFacilityList = async ({ page = 1, limit = PER_PAGE_LIMIT }) => {
        try {
            setLoading(true);
            ...
            toast.error("Unable to fetch the facility lists ....");
    ...
    useEffect(() => {
        getFacilityList({ page: 1, limit: PER_PAGE_LIMIT });
    }, []);
    ...
    const deleteFacility = async (id: string) => {
        try {
            ...
            toast.error("Facility cannot be deleted at this moment");
            console.error(exception);
        } finally {
            setLoading(false);
        }
            ...
            <div className="mx-auto max-w-screen-xl px-4 py-4 sm:px-6 lg:px-8">
                <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3 lg:gap-16">
                    ...
                    <div className="col-span-full lg:col-span-1 flex justify-center lg:justify-end">
                        <NavLink className="bg-green-800 mt-3 text-center py-2 px-2 text-white rounded w-[200px]" to="/admin/facility/create">
                            Create Facility
                        </NavLink>
                    </div>
                </div>
                ...
                <div className="rounded-lg border border-gray-200 mt-8">
                        ...
                        <table className="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
                            <thead className="ltr:text-left rtl:text-right bg-black">
                                <tr>
                                    ...
                                    <th className="whitespace-nowrap px-4 py-2 font-medium text-white">Add on Price</th>
                                ...
                                {loading ? (
                                    ...
                                    data.map((facility: any, index: number) => (
                                        <tr className="odd:bg-gray-50" key={index}>
                                            <td className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">{facility.name}</td>
                                            <td className="whitespace-nowrap px-4 py-2 font-medium">{facility.description}</td>
                                            <td className="whitespace-nowrap px-4 py-2 font-medium">{facility.adonprice}</td>
                                            {/* <td className="whitespace-nowrap px-4 py-2 font-medium">{facility.status}</td> */}
                                            <td className="whitespace-nowrap px-4 py-2 font-medium">
                                                <TableActionButtons
                                                    editUrl={"" + facility._id}
                                                    rowId={facility._id as string}
                    ...
                    {!loading && (
                        <PaginationComponent fetchCall={getFacilityList} pagination={pagination} />
                    )}
                </div>
            </div>
        </section>
